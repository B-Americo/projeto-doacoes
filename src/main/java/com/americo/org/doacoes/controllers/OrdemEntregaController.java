package com.americo.org.doacoes.controllers;

import com.americo.org.doacoes.model.entities.Categoria;
import com.americo.org.doacoes.model.entities.OrdemEntrega;
import com.americo.org.doacoes.model.entities.Usuario;
import com.americo.org.doacoes.repository.OrdemEntregaRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;
import org.tinylog.Logger;

import java.util.List;
import java.util.Optional;

@RestController
@RequestMapping("orders")
public class OrdemEntregaController {
    @Autowired
    private OrdemEntregaRepository ordemEntregaRepository;


    @PostMapping
    public @ResponseBody OrdemEntrega novoCadastro( OrdemEntrega ordemEntrega) {
        try {
            ordemEntregaRepository.save(ordemEntrega);
            Logger.info("salvo no bd corretamente!");
            return ordemEntrega;
        } catch (Exception e) {
            Logger.error("Erro no cadastro!" + e);
            return null;
        }
    }

    @GetMapping
    public Iterable<OrdemEntrega> consultarTodos() {
        try {
            Logger.info("Consulta de todos os cadastros no bd realizada corretamente");
            return ordemEntregaRepository.findAll();
        }catch (Exception e) {
            Logger.error("Erro: " + e);
            return null;
        }
    }

    @GetMapping(path = "/{id}")
    public Optional<OrdemEntrega> consultarPorId(@PathVariable Long id) {
        try {
            Logger.info("Consulta por id realizada com sucesso");
            return ordemEntregaRepository.findById(id);
        }catch (Exception e){
            Logger.error("Erro: " + e);
            return null;
        }
    }

    @PutMapping
    public @ResponseBody OrdemEntrega alterar( OrdemEntrega ordemEntrega) {
        try {
            ordemEntregaRepository.save(ordemEntrega);
            Logger.info("Update realizado no bd corretamente!");
            return ordemEntrega;
        }catch (Exception e){
            Logger.error("Erro no update!" + e);
            return null;
        }
    }

    @DeleteMapping(path = "/{id}")
    public void deletar(@PathVariable Long id){
        try {
            ordemEntregaRepository.deleteById(id);
            Logger.info("deletado corretamente");
        }catch (Exception e){
            Logger.info("Erro ao deletar" + e);
        }
    }
}